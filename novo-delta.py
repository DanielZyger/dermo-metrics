...
class SimpleFingerprintDetector:
    ...
    def visualize(self, deltas, cores, orientation, coherence, poincare, block_size):
        ...
        # Desenha linhas entre deltas e cores PRIMEIRO (para ficarem atrás)
        for delta in deltas:
            for core in cores:
                # Linha do centro do delta até o centro do core
                cv2.line(result, (delta['x'], delta['y']), (core['x'], core['y']), 
                        (0, 255, 0), 2)  # Linha verde
        
        for delta in deltas:
            # Desenha triângulo vermelho SEMI-TRANSPARENTE para delta (tamanho reduzido)
            x, y = delta['x'], delta['y']
            pts = np.array([[x, y-20], [x-17, y+12], [x+17, y+12]], np.int32)
            
            # Cria uma camada temporária apenas para o preenchimento
            overlay = result.copy()
            cv2.fillPoly(overlay, [pts], (0, 0, 255))
            
            # Aplica transparência no preenchimento (alpha=0.4 = 40% opaco, 60% transparente)
            alpha = 0.4
            cv2.addWeighted(overlay, alpha, result, 1 - alpha, 0, result)
            
            # AGORA desenha a borda do triângulo OPACA por cima (100% visível)
            cv2.polylines(result, [pts], True, (0, 0, 255), 3)
        
        for core in cores:
            # Desenha círculo azul para core (tamanho reduzido)
            x, y = core['x'], core['y']
            cv2.circle(result, (x, y), 20, (255, 0, 0), 3)
